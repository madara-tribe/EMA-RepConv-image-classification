...
def inference(onnx_file_name, model_type='stu'):
    ...
    if 'llm' in model_type:
        outputs = ort_session.run(None, {input_name: img_in.astype(np.float32)})[1]
    else:
...
def load_trained_model(cfg, device, weight_path, model_type):
    if 'llm' in model_type:
...
def Convert_ONNX(model, model_type): 
    H = W = 260
    # set the model to inference mode 
    model.eval() 
    dummy_input = torch.randn(1, 3, H, W, requires_grad=True)  
    onnx_file_name = "repconv{}_{}.onnx".format(cfg.input_size, cfg.input_size)
    # Export the model   
    ...
    inference(onnx_file_name, model_type)
...
if __name__=="__main__":
    ...
    model = load_trained_model(cfg, device, weight_path, model_type)
    Convert_ONNX(model, model_type)
